...
export const postSignup = async (req, res) => {
	try {
		// asocio un carrito al usuario recién creado para
		const user = req.user._id;
		const cartId = await cartsModel.createUserCart(user);
	...
	try {
		...
		if (req.session.login) {
			...
			if (userEmail !== '' && userEmail != null) {
				...
				const userCart = await cartsModel.getUserCart(userInfo._id);
